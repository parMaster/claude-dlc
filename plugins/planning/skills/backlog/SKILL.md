...
Three frontmatter fields, written once and rewritten only as **Appending** below allows, plus an optional
`ticket`:
...
- **`ticket: PROJ-123`** — optional; the tracker ticket filed for an item substantial enough to need one.
  Usually added after the item exists, whenever the user asks — the one field that can be added at any
  time. The item file stays the source of truth for the repo; the ticket is where the work is scheduled.
  Two items naming the same ticket are the same item.
...
`/planning:backlog --all` walks every current item to a disposition. Check this mode before slug handling.
...
`/planning:backlog <slug>` names one item: `docs/backlog/<slug>.md`, the file name without its extension. Read
...
3. Report every item, one line each — `yes` first, then `later`, then `no`, oldest `added` first within
   each group. Include `where` and `ticket` for the items that have them. Do not editorialize; the item already carries
   its reasoning.
...
1. Detect the repository's default branch: read `git symbolic-ref refs/remotes/origin/HEAD` and strip
   the `refs/remotes/origin/` prefix. It fails when `origin/HEAD` was never set (a repo that was not
   cloned); then ask the remote with `git ls-remote --symref origin HEAD` and take the name from its
   `ref: refs/heads/<name>` line. Never guess from common names — a repo's default may be `develop`.
2. Compare it with `git branch --show-current`. If no default branch was found at all, say so and take
   the "unrelated" case below: there is nothing to compare against, so the answer must come from the
   user rather than from a guess.
   - **On the default branch** — write in place.
   - **On a branch whose work the item is about** — the item's `where` path is in
     `git diff --name-only origin/<default>...HEAD`. That range covers every commit not yet on the
     default branch, including parent branches in a stack, so code introduced by any of them counts.
     Write here without asking: the code only exists on this branch, so the item travels with it.
   - **Anywhere else** — no `where`, or a path the branch has not touched. Report the current branch by
     name, or `detached HEAD at <sha>` when the command returns nothing, and ask whether to write in
     this checkout anyway. On yes, write here. On no, change nothing: do not create a branch, a
     worktree, or a temp checkout, do not switch the current checkout, and do not pick a destination
